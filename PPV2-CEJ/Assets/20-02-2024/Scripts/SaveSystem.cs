...
public class SaveSystem : MonoBehaviour
{
    ...
    //Singleton-este método asegura que solo exista una instancia de la clase
    ...
    private void Start()
    {
        // Función que llama desde el inicio a la función SaveToJSON la cual guarda el nombre del dato (LeccionYeah) y el tipo de dato.
        SaveToJSON("LeccionYeah", data);

        // Además de llamar a subject que se encarga de cargar datos de la fución LoadFromJSON a el tipo de objeto donde se guardara SubjectContainer.
        subject = LoadFromJSON<SubjectContainer>("SubjectYeah");
    }

    /// <summary>
    /// Está funcion esta encargada de almacenar objetos en archivos JSON.
    /// </summary>
    ...
    public void SaveToJSON(string _fileName, object _data)
    {
        ...
        if (_data != null)
        {
            ...
            if (JSON_data.Length != 0)
            {
                ...
                /*/Finalmente, el JSON se escribe en el archivo especificado utilizando File.WriteAllText el cual escribe toda la información almacenada./*/
                File.WriteAllText(filePath, JSON_data);

                /*/Mensaje que se manda para poder indicar que se guardo la información en la variale filePath la cual indica las carpetas donde se guardan./*/
    ...
    public T LoadFromJSON<T>(string _fileName) where T : new()
    {
        ...
        if (File.Exists(path))
        {
            ...
            /*/Si el archivo existe, se lee su contenido y se almacena en la variable JSON_data. /*/
